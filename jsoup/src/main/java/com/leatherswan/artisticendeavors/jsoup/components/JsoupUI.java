package com.leatherswan.artisticendeavors.jsoup.components;
...
import com.leatherswan.artisticendeavors.jsoup.utils.JsoupUtil;
...
public class JsoupUI {
    ...
    private void pagePreviewDTOFromFile() {
        ...
        try {
            ...
            doc = Jsoup.parse(in, null, JsoupUtil.getBaseUri(linkUrl));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private List<JsoupImage> getImages(Document doc) {
        ...
        for (Element media : elements) {
            if (media.tagName().equals("img")) {
                ...
                img.setAlt(JsoupUtil.trim(media.attr("alt"), 60));
                img.setHeight(JsoupUtil.attrIntToNull(media.attr("height")));
                img.setWidth(JsoupUtil.attrIntToNull(media.attr("width")));
                images.add(img);
            }
        }
        return images;
    ...
    private void displayLInks() {
        ...
        for (Element link : links) {
            print(" * a: <%s>  (%s)", link.attr("abs:href"), JsoupUtil.trim(link.text(), 35));
        }
    }
        ...
        for (Element src : media) {
            if (src.tagName().equals("img"))
                print(" * %s: <%s> %sx%s (%s)",
                        ...
                        JsoupUtil.trim(src.attr("alt"), 60));
            else
