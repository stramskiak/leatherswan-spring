...
import com.leatherswan.artisticendeavors.mvc.containers.UserQuery;
...
import com.leatherswan.artisticendeavors.mvc.containers.Pager;
...
/*
	...
	public String processFindForm(UserQuery userQuery, BindingResult result, Model model, HttpServletRequest request) {
		...
		if (results.size() > 1) {
			...
			return "redirect:/products/" + product.getContactId();
		}
	}
*/
...
*/
	...
	public String productPage(@PathVariable("id") String id, Model model) {
		...
		Product found = productService.getProduct(Long.valueOf(id));
	...
	private List<ProductDTO> createDTOs(List<Product> products) {
		...
		for (Product product : products) {
			dtos.add(ProductUtils.productToProductDTO(product));
		}
		return dtos;
	}
